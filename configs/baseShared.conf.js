exports.config = {
    ...
    before: function (capabilities, specs) {
        ...
        //NPM Fetch library to make API calls
        ...
        browser.overwriteCommand('click', function (origClickFunction, {
            FORCE = false,
            TIMEOUT = 10000
        } = {}) {
            this.waitForExist({
            ...
            console.warn('WARNING: Using force click for', this.selector)
            try {
                browser.execute("arguments[0].click();", this)
            } catch (err) {
                throw err
            }
        }, true)

        // Over writing "setValue" function
        browser.overwriteCommand('setValue', function (origsetValueFunction, text, TIMEOUT = 10000) {
            this.waitForExist({
                timeout: TIMEOUT,
                timeoutMsg: 'Given element ' + this.selector + ' does NOT EXIST after ' + TIMEOUT + ' MiliSeconds'
            });
            browser.pause(400)
            try {
                return origsetValueFunction(text)
            } catch (err) {
                throw err
            }
